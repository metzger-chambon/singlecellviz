...
As you can notice, it has a similar structure than the file given as input of `run_app()` (described in [Database vignette](database.html)). This is because the input file given to `run_app()` is read, and saved in the variable `studies`.
...
-   `selected_study`, the number (positional value) of the current selected study
...
- follow [best practice](https://frank.taillandier.me/2014/12/14/un-modele-de-branches-git-efficace/) for merging to a release branch 
- run `golem::set_golem_version("X.X.X")` with your new version numbers
- describe changes in `NEWS.md`
- follow [best practice](https://frank.taillandier.me/2014/12/14/un-modele-de-branches-git-efficace/) for merging to the master branch
