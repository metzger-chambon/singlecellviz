...
They are basically wrappers for `Seurat::FindAllMarkers()`, `Seurat::FindMarkers()` and `Seurat::AggregateExpression()`.
...
yaml_content <- list("title" = "Study 1",
     "description" = "Some description",
     "doi" = "XX.XXXX/somealphanum",
     "date" = "DD/MM/YYYY")

yaml::write_yaml(yaml_content, file = "config.yaml")
```
...
It automatically searches for all available `config.yaml`, `object.rds`and `tiledb/` in each subfolder of the given db_dir (e.g. `db/`). 
