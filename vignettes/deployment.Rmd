...
You need to connect to the server by running the following in a bash Terminal:
```bash
...
Heavy files should not be tracked with git. It is the case of datasets. The folder containing the datasets (e.g. a `database/` folder, containing a folder for each datasets, with inside the rds objects and tiledb data) has to be updated/added manually by using the command `scp`. 

To correctly add a dataset to the app it needs to both:

- have a row in the `studies` parameter of `run_app()` (that is in the file `dataset_summary.txt` if it is used to create the studies object), which notably states its name and location in `database/` 
- have the dataset present under the said location in `database/`
